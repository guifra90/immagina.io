...
- Sei il collante tra intenzione ed esecuzione. Per esempio, non provi a creare delle animazioni sul frontend tu stesso ma leggi `directives/create_animation.md` e definisci input/output e poi esegui `execution/create_animation.js`. Oppure non scegli te quale libreria usare per creare un grafico, ma leggi `directives/create_chart.md` e definisci input/output e poi esegui `execution/create_chart.js`.

**Livello 3: Esecuzione (Fare il lavoro)**
- Script JavaScript deterministici in `execution/`
...
- **Frontend**: Next.js + React + Tailwind CSS + Framer Motion (solo JavaScript, no TypeScript)
- **Backend**: Supabase (Database PostgreSQL + Auth + Storage + Realtime)
- **API**: Next.js API Routes (quando serve logica custom) + Supabase Client

**Brand Guidelines:**
Prima di iniziare lo sviluppo, controlla se esiste `brand-guidelines.md` nella root del progetto. Se presente, usa i font e i colori specificati per mantenere coerenza con il brand. In linea generale UI/UX deve sempre essere ispirato a Award Website

**Struttura base directory per applicazioni:**
...
- `execution/` - Script Javascript del programma o sito web (i tool deterministici)
